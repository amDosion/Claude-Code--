...
### 3.6 实时通知 — `src/hooks/useSlaveNotifications.ts`

**状态**：新建。

当 Master CLI 收到 slave 会话事件时，在底部 footer 实时弹出 toast 通知：

| 事件 | 通知内容 | 优先级 | 超时 |
|------|----------|--------|------|
| `prompt`（slave 开始处理） | `[slave-name] ▶ 开始处理: <内容>` | medium | 4s |
| `tool_start`（工具执行） | `[slave-name] 工具: <工具名>` | low | 3s |
| `done`（一轮完成） | `[slave-name] ✓ 任务完成` | medium | 5s |
| `error`（错误） | `[slave-name] ✗ 错误: <原因>` | high | 8s |
| `stream`（AI 输出片段） | —（跳过，太频繁） | — | — |
| `tool_result`（工具结果） | —（跳过，太频繁） | — | — |

**折叠机制**：相同 slave 的连续同类通知会折叠为计数形式，如 `[cli-abc] ✓ 任务完成 (3)`。

**实现原理**：
- 监听 `AppState.pipeIpc.slaves` 变化
- 用 `useRef` 记录每个 slave 的上次已读 history 长度
- 检测到新条目时调用 `addNotification()` 推入通知队列
- 通知在 `PromptInput/Notifications.tsx` 组件中渲染（已有基础设施）

### 3.7 状态定义 — `src/state/AppStateStore.ts`
...
| `src/hooks/useSlaveNotifications.ts` | Master 侧实时 toast 通知：slave 事件触发底部弹窗 |
